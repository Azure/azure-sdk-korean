...
위의 고려 사항은 언어마다 정도가 다를 수 있으므로, 설계 단계 초기에 특정 언어에 대해 승인된 의존성 및 지침을 확인하는 것이 중요합니다. (또한 드문 경우지만, 상당한 검토를 거친 후 이사회가 그렇게 함으로써 고객을 지속적으로 지원하는 데 따른 위험이 최소화된다고 판단할 경우, Azure SDK 아키텍처 위원회는 추가적인 서드파티 라이브러리에 대한 하드 의존성을 갖도록 결정할 수도 있습니다.)
